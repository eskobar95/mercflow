...
/**
 * Validates a Medusa store id before it is passed to SQL session configuration.
 */
...
/**
 * Sets transaction-local `app.store_id` for PostgreSQL RLS policies (ADR-005).
 * Uses parameterized `set_config` instead of string-interpolated SET LOCAL.
 */
export async function setLocalAppStoreId(
  transactionManager: unknown,
  storeId: string
): Promise<void> {
  assertMedusaStoreId(storeId)
  const runner = resolveSqlExecutor(transactionManager)
  await runner.execute(`SELECT set_config('app.store_id', ?, true)`, [storeId])
}
...
/**
 * Runs `fn` inside a DB transaction with `app.store_id` set for RLS tenant_isolation policies.
 */
export async function runWithTenantScope<T>(
  ...
  return baseRepository.transaction(async (transactionManager: unknown) => {
    await setLocalAppStoreId(transactionManager, storeId)
