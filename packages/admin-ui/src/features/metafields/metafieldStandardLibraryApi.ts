...
export async function listStandardLibraryDefinitions(params: {
  ...
  const response = await fetch(
    `${base}/admin/metafield-definitions/standard-library?${search.toString()}`,
    {
      method: "GET",
...
export type ActivateStandardLibraryResult = {
  activated: MetafieldDefinitionDto[]
  skippedKeys: string[]
}

export async function activateStandardLibraryDefinitions(params: {
  vertical: MetafieldLibraryVertical
  definitionIds: string[]
}): Promise<ActivateStandardLibraryResult> {
  const base = requireBackendUrl()
  const response = await fetch(`${base}/admin/metafield-definitions/activate-standard`, {
    method: "POST",
    headers: buildMedusaAdminJsonHeaders(),
    credentials: "include",
    body: JSON.stringify({
      vertical: params.vertical,
      definition_ids: params.definitionIds,
    }),
  })
  ...
  const activated = json.metafield_definitions
    .map((row) => parseMetafieldDefinitionRow(row))
    .filter((row): row is MetafieldDefinitionDto => row !== null)

  const skippedKeys = Array.isArray(json.skipped_keys)
    ? json.skipped_keys.filter((key): key is string => typeof key === "string")
    : []

  return { activated, skippedKeys }
}
