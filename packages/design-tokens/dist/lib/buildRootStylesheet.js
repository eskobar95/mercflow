...
export function buildRootStylesheet() {
    const colorLines = linesForFlatPrefix(flattenRootStringTree(colorTree), "--color-");
    const spacingLines = Object.entries(spacingScale).map(([k, v]) => `  --spacing-${k.replace(".", "-")}: ${v};`);
    const fontFamilyLines = Object.entries(fontFamily).map(([k, v]) => `  --font-family-${k}: ${v};`);
    const fontSizeLines = Object.entries(fontSize).map(([k, v]) => `  --font-size-${k}: ${v};`);
    const fontWeightLines = Object.entries(fontWeight).map(([k, v]) => `  --font-weight-${k}: ${v};`);
    const lineHeightLines = Object.entries(lineHeight).map(([k, v]) => `  --line-height-${k}: ${v};`);
    const letterSpacingLines = Object.entries(letterSpacing).map(([k, v]) => `  --letter-spacing-${k}: ${v};`);
