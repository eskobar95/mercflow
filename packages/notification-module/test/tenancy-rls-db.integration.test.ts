...
describe.skipIf(!dbIntegrationReady)("email_configs RLS DB integration", (): void => {
  it("has RLS enabled with email_configs_tenant_isolation policy", async (): Promise<void> => {
    await assertTableRls("email_configs", "email_configs_tenant_isolation")
  })

  it("returns zero cross-tenant rows without app.tenant_id", async (): Promise<void> => {
    await assertZeroCrossTenant("email_configs")
  })
})

describe.skipIf(!dbIntegrationReady)("email_deliveries RLS DB integration", (): void => {
  it("has RLS enabled with email_deliveries_tenant_isolation policy", async (): Promise<void> => {
    await assertTableRls("email_deliveries", "email_deliveries_tenant_isolation")
  })

  it("returns zero cross-tenant rows without app.tenant_id", async (): Promise<void> => {
    await assertZeroCrossTenant("email_deliveries")
  })
})
