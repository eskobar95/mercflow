...
export const ENV_VAR_NAME = "MERCFLOW_ENCRYPTION_KEY"
...
/**
 * AES-256-GCM encrypt/decrypt for payment provider secret keys.
...
export default class EncryptionService {
  ...
  constructor(options?: EncryptionServiceOptions) {
    ...
    if (resolved === null) {
      throw new MedusaError(
        ...
        `${ENV_VAR_NAME} must be set to a 64-character hex string (32 bytes) so payment provider credentials can be encrypted at rest`
      )
    }
  ...
  decrypt(encoded: string): string {
    if (!encoded.startsWith(VERSION_PREFIX)) {
      throw new MedusaError(
        MedusaError.Types.INVALID_DATA,
        "Stored payment credentials have an unknown format or version prefix"
      )
    }
    ...
    if (combined.length < IV_LENGTH_BYTES + AUTH_TAG_LENGTH_BYTES) {
      throw new MedusaError(
        MedusaError.Types.INVALID_DATA,
        "Stored payment credentials are truncated or corrupted"
      )
    }
    ...
    try {
      ...
      throw new MedusaError(
        MedusaError.Types.INVALID_DATA,
        "Stored payment credentials could not be decrypted (wrong key or tampered data)"
      )
    }
  }
}
