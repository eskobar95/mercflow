...
describe.skipIf(!dbIntegrationReady)("payment_provider_config RLS DB integration", (): void => {
  it("has RLS enabled with payment_provider_config_tenant_isolation policy", async (): Promise<void> => {
    const hasStoreId = await tableHasColumn("payment_provider_config", "store_id")
    if (!hasStoreId) {
      return
    }
    await assertTableRls(
      "payment_provider_config",
      "payment_provider_config_tenant_isolation"
    )
  })
  ...
  it("returns zero cross-tenant rows without app.tenant_id", async (): Promise<void> => {
    ...
    await assertZeroCrossTenant("payment_provider_config")
  })
})
